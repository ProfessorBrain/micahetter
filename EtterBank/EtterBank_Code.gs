...
const SHEET_ID = "1g-tFJt1NzFhQj2XaQCbQYGsC1d1Zk737zo-hITBDlvY"; // Optional: paste your Sheet ID here if using a standalone script project.
...
function canonicalUser_(username) {
  ...
  return u; // fall back to what was provided
}

const SHEET_ACCOUNTS = "Accounts";
const SHEET_LEDGER = "Ledger";
const NOTIFY_EMAIL = "micahetter@gmail.com";
...
function applyGoalAction(username, pass, action, amount) {
  username = canonicalUser_(username);
  pass = (pass || "").toString();
...
function applyPrimaryAction(username, pass, action, amount) {
  username = canonicalUser_(username);
  pass = (pass || "").toString();
  action = (action || "").toString().trim().toLowerCase();
  ...
  if (action === "withdraw") {
    if (rec.primary + 1e-9 < amt) throw new Error("Withdrawal exceeds available primary balance.");
    rec.primary = Math.round((rec.primary - amt) * 100) / 100;
  } else {
    rec.primary = Math.round((rec.primary + amt) * 100) / 100;
  }

  // Write back primary balance:
  acctSheet.getRange(row, 2).setValue(rec.primary); // B
  SpreadsheetApp.flush();

  // Append ledger row (no match for primary actions)
  ledgerSheet.appendRow([
    new Date(),
    username,
    action === "deposit" ? "primary_deposit" : "primary_withdraw",
    amt,
    0,
    0,
    rec.primary,
  ...
  let email_error = "";
  try {
    const subject = `EtterBank update: ${username} updated their primary account`;
    const body =
      `User: ${username}\n` +
      `Action: ${action}\n` +
      `Amount: $${amt.toFixed(2)}\n` +
      `Primary balance (after): $${Number(rec.primary).toFixed(2)}\n` +
      `Goal-Directed balance: $${Number(rec.goal).toFixed(2)}\n` +
      `Time: ${new Date().toLocaleString()}\n\n` +
      `This update was submitted from the EtterBank web app.`;
    MailApp.sendEmail(NOTIFY_EMAIL, subject, body);
    email_sent = true;
  } catch (e) {
    email_error = (e && e.message) ? String(e.message) : "Email failed.";
  }

  const snap = getSnapshot(username, pass);
  snap.email_sent = email_sent;
  if (email_error) snap.email_error = email_error;
  return snap;
}


function requireAuth_(username, pass) {
  const canon = canonicalUser_(username);
  const u = USERS[canon];
